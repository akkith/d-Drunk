...
public final class Nakanaka extends BotSkeleton {
    ...
    public Cards requestingGivingCards() {
		...
		try {
			...
			if(diffrank > 0) {
				// ダイヤの3は渡さない HT 各種３は渡さない
				singleCards.remove(Card.D3,Card.H3,Card.C3,Card.S3);
	...
	Strategy[] strategyList = {
			...
			//HT  new LessThanThreeCardsStrategy(),
			//HT  new GiveUpStrategy(),
			new FugouStrategy(),
			new HeiminStrategy(),
			new HinminStrategy(),
	...
	/**
	 * カード集合から、JOKER と組み合わせ役のカードを除く、HTさらに３を除く
	...
	private Cards removeCombinationMelds(Cards cards) {
		...
		//HT 3を除く
		cards = cards.remove(Card.C3,Card.D3,Card.H3,Card.S3);
